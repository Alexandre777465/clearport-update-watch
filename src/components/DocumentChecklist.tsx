import { Card } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import type { DocumentChecklistItem } from "@/lib/api";
import { CheckCircle2, Circle, AlertCircle } from "lucide-react";

export function DocumentChecklist({
  items,
  uploadedDocTypes = [],
}: {
  items: DocumentChecklistItem[];
  uploadedDocTypes?: string[];
}) {
  const required = items.filter((d) => d.required);
  const optional = items.filter((d) => !d.required);

  const isUploaded = (doc: DocumentChecklistItem) =>
    uploadedDocTypes.some((t) =>
      t.toLowerCase().includes(doc.document.toLowerCase().slice(0, 8)),
    );

  const missingRequired = required.filter((d) => !isUploaded(d));

  return (
    <div className="space-y-4">
      {missingRequired.length > 0 && (
        <div className="flex items-center gap-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
          <AlertCircle className="h-3.5 w-3.5 shrink-0" />
          {missingRequired.length} required document{missingRequired.length !== 1 ? "s" : ""} not yet collected
        </div>
      )}

      <div>
        <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
          Required from your supplier
        </p>
        <div className="space-y-2">
          {required.map((doc) => {
            const uploaded = isUploaded(doc);
            return (
              <Card key={doc.document} className={`p-3 ${uploaded ? "opacity-60" : ""}`}>
                <div className="flex items-start gap-2">
                  {uploaded
                    ? <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-green-600" />
                    : <Circle className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
                  }
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium">{doc.document}</span>
                      <Badge variant="outline" className="text-xs border-red-200 bg-red-50 text-red-700">
                        Required
                      </Badge>
                      {uploaded && (
                        <Badge variant="outline" className="text-xs border-green-200 bg-green-50 text-green-700">
                          Uploaded
                        </Badge>
                      )}
                    </div>
                    <p className="mt-0.5 text-xs text-muted-foreground">{doc.reason}</p>
                  </div>
                </div>
              </Card>
            );
          })}
        </div>
      </div>

      {optional.length > 0 && (
        <div>
          <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            Recommended
          </p>
          <div className="space-y-2">
            {optional.map((doc) => (
              <Card key={doc.document} className="p-3">
                <div className="flex items-start gap-2">
                  <Circle className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
                  <div className="min-w-0">
                    <span className="text-sm font-medium">{doc.document}</span>
                    <p className="mt-0.5 text-xs text-muted-foreground">{doc.reason}</p>
                  </div>
                </div>
              </Card>
            ))}
          </div>
        </div>
      )}
    </div>
  );
}
